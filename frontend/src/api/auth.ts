 ...
 */
export async function registerAccount(payload: RegisterRequest): Promise<LoginResponse> {
...
export async function refresh(): Promise<{
  ...
  const { data } = await api.post<{ accessToken: string; expiresIn: number }>("/auth/refresh", {})
  return data
}
 ...
 */
export async function updateMyProfile(payload: UpdateProfileRequest): Promise<Member> {
